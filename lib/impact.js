...
function analyze(root, id) {
  ...
  // 의존 관계가 아예 기록돼 있지 않으면 "영향 없음"이 아니라 "모른다"이다.
  // 거짓 안심이 아무 정보도 없는 것보다 나쁘다.
  const anyRecorded = (state.backlog || []).some((b) => (b.dependsOn || []).length);
  const depsUnknown = !anyRecorded && (state.backlog || []).length > 1;
  ...
  return { id, row, status, dependents: deps, depsUnknown, contract, live, reversible, cost };
}

/** 백로그 전체. 변경을 이야기하려면 정확한 현재 목록이 먼저다. */
function list(root) {
  const state = require('./state').load(root);
  const rows = state.backlog || [];
  return {
    rows: rows.map((b) => ({ ...b, spec: specFilled(root, b.id) })),
    depsRecorded: rows.some((b) => (b.dependsOn || []).length),
    current: state.currentFeature ? state.currentFeature.id : null,
  };
}

module.exports = { STATE, analyze, classify, dependents, specFilled, hasContract, list };
