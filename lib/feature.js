...
function complete(root) {
  return state.update(root, (s) => {
    ...
    // 게이트를 부르지 않으면 아무 일도 안 일어나던 구멍을 막는다.
    // 프로듀서가 건너뛰어도 여기서 걸린다 — 프롬프트가 아니라 기계다.
    const required = [
      ['securityPassed', 'devoffice gate security', '되돌릴 수 없는 것 (권한·비밀키)'],
      ['evidenceChecked', 'devoffice gate evidence', '인수 조건마다 근거 대조'],
    ];
    const skipped = required.filter(([flag]) => !cf.gates[flag]);
    if (skipped.length) {
      ui.header('아직 마무리할 수 없어요', cf.id);
      ui.line('확인을 건너뛴 게 있어요.');
      ui.line();
      skipped.forEach(([, cmd, why]) => ui.line(` · ${why}`));
      ui.line();
      ui.line('이건 나중에 고칠 수 없는 종류라 먼저 봐야 해요.');
      ui.next('wait', `${skipped.map(([, c]) => c).join(' · ')} 를 먼저 실행하세요`);
      ui.flush();
      process.exit(2);
    }
 ...
 * 진행 중인 기능을 접는다.
 *
 * 진행 잠금(D53)은 "확인 없이 다음으로 넘어가는 것"을 막는 장치지 **감옥이 아니다.**
 * 사용자가 "이건 나중에 할래요"라고 말할 길이 없으면 제품 전체가 거기서 멈춘다.
 *
 * 만든 서류철은 지우지 않는다 — 나중에 다시 시작할 때 그대로 쓴다.
 */
function cancel(root, reason) {
  return state.update(root, (s) => {
    const cf = s.currentFeature;
    if (!cf) ui.fail('진행 중인 기능이 없어요.');
    ...
    journal.add(root, 'feature.cancel', cf.title, { id: cf.id, reason: reason || '' });
    s.currentFeature = null;

    ui.header('잠시 접어뒀어요', cf.id);
    ui.line(`${ui.withJosa(cf.title, '은')} 나중에 다시 하실 수 있어요.`);
    ui.line('지금까지 정한 건 그대로 남아 있어요.');
    ui.next('turn', '다음에 뭘 할지 말씀해주세요');
    ui.flush();
    return { id: cf.id, title: cf.title };
  });
}

/**
 * 만들어진 기능을 수정하러 다시 연다 (CHANGE.md §3 상태③).
 *
 * **다시 미검증 상태가 된다.** 고친 뒤 사용자가 또 확인해야 한다 —
 * "고쳐뒀어요"로 끝내면 안 고쳐진 채로 넘어간다.
 */
function reopen(root, id, reason) {
  requireUnlocked(root);
  return state.update(root, (s) => {
    const row = (s.backlog || []).find((b) => b.id === id);
    if (!row) ui.fail(`${id}는 목록에 없는 기능이에요.`);

    const f = schema.newFeature(id, row.title);
    f.reopenedFrom = row.status;
    f.reason = reason || '';
    s.currentFeature = f;
    s.phase = 'FEATURE_LOOP';
    row.status = 'in_progress';

    journal.add(root, 'feature.reopen', row.title, { id, from: f.reopenedFrom, reason: reason || '' });

    ui.header(`${row.title} 다시 열었어요`, id);
    ui.line('지금까지 만든 건 그대로 있어요. 바뀌는 부분만 고칠게요.');
    ui.line();
    ui.line('고치고 나면 한 번 더 확인해주셔야 해요.');
    ui.next('wait', '무엇을 바꿀지 정리할게요');
    ui.flush();
    return f;
  });
}
...
module.exports = { lock, requireUnlocked, start, pass, verify, complete, cancel, reopen, show };
