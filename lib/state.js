...
1. **먼저 \`devoffice state show\` 로 지금 어디까지 왔는지 확인한다.** 추측하지 않는다
2. **모든 응답은 👉(사용자 차례) / ⏳(작업 중) / 🔗(밖에서 확인) 중 하나로 끝난다**
3. **기술 용어를 쓰지 않는다.** API→"서버", 배포→"인터넷에 올리기"
4. **사용자가 눈으로 확인하기 전에는 다음 기능을 시작하지 않는다**
5. **사용자가 모르는 것을 만들지 않는다.** "있으면 좋은 것"은 백로그에 제안만
6. 오래 걸리는 일 전에는 **"화면이 조용해도 정상이에요"**를 알린다
...
\`\`\`
devoffice state show / feature / task / gate / doctor
\`\`\`
...
const PHASE_KO = {
  INTAKE: '무엇을 만들지 이야기하는 중',
  NAMING: '이름 정하는 중',
  PRD: '제품 정리하는 중',
  DESIGN: '어떻게 만들지 정하는 중',
  SETUP: '준비물 갖추는 중',
  SKELETON: '첫 화면 만드는 중',
  BACKLOG: '만들 것 목록 정하는 중',
  FEATURE_LOOP: '기능 만드는 중',
  OPERATE: '운영 중',
};
...
function summary(root) {
  ...
  ui.header(name, total ? `${done}/${total}` : undefined);
  ui.line(`지금  ${PHASE_KO[s.phase] || s.phase}`);
  ...
  if (cf) {
    ...
    ui.line();
    ui.line(`진행 중  ${cf.title}${t.length ? `  (${td}/${t.length})` : ''}`);
