...
/**
 * 인수 조건 목록. **state 가 유일한 출처다.**
 * 마크다운 폴백은 예전 프로젝트용으로만 남긴다.
 */
function acceptanceOf(root, id) {
  const cf = state.load(root).currentFeature;
  if (cf && cf.id === id && (cf.acceptance || []).length) {
    return { ids: cf.acceptance.map((a) => a.id), rows: cf.acceptance, fromState: true };
  }
  const specFile = path.join(P.paths(root).feature(id), 'spec.md');
  const ids = md.acIds(md.sections(md.read(specFile) || '')['인수 조건'] || '');
  return { ids, rows: ids.map((x) => ({ id: x, text: '' })), fromState: false };
}
...
function checkSpec(root, id) {
  ...
  // PROTOCOL §5.1 — 검증 가능한 인수 조건. state 가 출처다.
  const { ids: acs, fromState } = acceptanceOf(root, id);
  if (acs.length === 0) {
    out.push(problem(
      '인수 조건이 없어요.',
      '무엇을 확인할지 정해야 해요.\n' +
    ...
    out.push(problem(
      '인수 조건이 문서에만 있어요.',
      'devoffice ac add 로 등록해주세요. 문서에만 있으면 게이트가 매번 글을 읽어야 해요.'
    ));
...
function checkPlan(root, id) {
  ...
  // 작업·AC매핑·준비물은 state.json 이 유일한 출처다.
  // 같은 걸 마크다운에도 두면 둘이 어긋나고, 그때 어느 쪽이 맞는지 알 수 없다.
  const cf = state.load(root).currentFeature;
  const stateTasks = (cf && cf.tasks) || [];
  const useState = stateTasks.length > 0;

  if (useState) return checkPlanFromState(root, id, cf, s);

  // --- 아래는 예전 프로젝트용 폴백 (state 에 작업이 아직 없을 때) ---
...
/**
 * 구조가 있는 것은 구조체로 검사한다 — 마크다운 파싱을 거치지 않는다.
 * 손으로 만든 파서는 이미 세 번 틀렸다(자리표시자 오인·표 판정·창 넘어감).
 */
function checkPlanFromState(root, id, cf, planSections) {
  const out = [];
  const tasks = cf.tasks || [];

  if (tasks.length > LIMITS.tasksPerFeature) {
    out.push(problem(
      `작업이 ${tasks.length}개예요. ${LIMITS.tasksPerFeature}개를 넘으면 기능이 너무 큽니다.`,
      '기능을 나누는 편이 안전해요.'
    ));
  ...
  const orphan = tasks.filter((t) => !t.owner || t.owner === 'unassigned');
  if (orphan.length) {
    out.push(problem(
      `담당이 없는 작업이 ${orphan.length}개 있어요 (${orphan.map((t) => t.id).join(', ')}).`,
      '담당 없는 작업은 아무도 안 합니다. --owner 로 배정해주세요.'
    ));
  }

  // 모든 AC 가 최소 하나의 작업에 매핑되어야 한다 (PROTOCOL §6.1)
  const specAcs = acceptanceOf(root, id).ids;
  const covered = new Set(tasks.flatMap((t) => t.ac || []));
  const missing = specAcs.filter((a) => !covered.has(a));
  if (missing.length) {
    out.push(problem(
      `${missing.join(', ')} 를 맡은 작업이 없어요.`,
      '이대로 만들면 그 조건은 아무도 구현하지 않습니다.\n' +
      '              devoffice task add ... --ac AC-4 처럼 연결해주세요.'
    ));
  }

  // 준비물 — 없으면 사용자가 확인할 방법이 없다
  const prep = cf.prep || {};
  const lacks = [];
  if (!prep.deeplink) lacks.push('확인하러 갈 링크');
  if (!prep.seed) lacks.push('예시 데이터');
  if (!prep.accounts || !prep.accounts.length) {
    // 로그인이 없는 제품이면 "불필요"를 명시해야 통과
    if (prep.accounts !== 'none') lacks.push('테스트 계정 (없으면 --accounts none)');
  }
  if (lacks.length) {
    out.push(problem(
      `검증 준비물이 빠졌어요: ${lacks.join(', ')}`,
      '없으면 사용자가 확인 자체를 할 수 없어요.\n' +
      '              devoffice prep set --link ... --seed ... --accounts ...'
    ));
  }

  // 구조가 없는 것만 마크다운에서 본다
  if (md.isEmpty(planSections['재사용할 것'])) {
    out.push(problem(
      '재사용할 것을 조사하지 않았어요.',
      '기존 코드를 확인하고, 없으면 "없음 (조사함)"이라고 적으세요.'
    ));
...
/**
 * "잘 된 것 같다"를 막는다 (PROTOCOL §5.3).
 *
 * CTO 가 AC 마다 **파일:라인 근거**를 대야 한다.
 * 근거를 못 대면 미구현으로 간주한다 — 에이전트의 "다 했습니다"를 믿지 않는 구조다.
 * 검사하지 않으면 이 규칙은 프롬프트에만 남고, 프롬프트는 언젠가 어겨진다.
 */
function checkEvidence(root, id) {
  const dir = P.paths(root).feature(id);
  const cf = state.load(root).currentFeature;
  const specAcsAll = acceptanceOf(root, id).ids;

  // 구조체가 있으면 그걸 본다
  if (cf && (cf.evidence || []).length) {
    const out = [];
    const rec = new Map(cf.evidence.map((e) => [e.ac, e]));
    const missing = specAcsAll.filter((a) => !rec.has(a));
    if (missing.length) {
      out.push(problem(`${missing.join(', ')} 에 대한 대조가 없어요.`,
        '인수 조건 하나도 빠짐없이 판정해야 해요. 빠진 건 확인 안 한 것입니다.'));
    }
    const failed = [...rec.values()].filter((e) => e.ok === false).map((e) => e.ac);
    if (failed.length) {
      out.push(problem(`${failed.join(', ')} 가 아직 안 됐어요.`,
        '고친 뒤에 다시 대조해야 해요. 이대로 사용자에게 보내면 안 됩니다.'));
    }
    const noProof = [...rec.values()]
      .filter((e) => e.ok !== false && !/[\w/.-]+\.\w+/.test(e.proof || ''))
      .map((e) => e.ac);
    if (noProof.length) {
      out.push(problem(`${noProof.join(', ')} 에 근거가 없어요.`,
        '"잘 됩니다"는 근거가 아니에요. 어느 파일 몇 번째 줄인지 적어야 해요.'));
    }
    return out;
  }

  // --- 폴백: 예전 프로젝트의 verification.md ---
  const file = path.join(dir, 'verification.md');
  const text = md.read(file);
  if (text === null) {
    return [problem(
      '증거 대조표가 없어요.',
      `${path.relative(root, file)} 에 인수 조건마다 어디에 만들었는지 적어야 해요.`
    )];
  }

  const out = [];
  const rows = md.table(md.sections(text)['증거 대조'] || text);
  const specAcs = md.acIds(md.sections(md.read(path.join(dir, 'spec.md')) || '')['인수 조건'] || '');

  // 표에서 AC 별 판정과 근거를 뽑는다
  const seen = new Map();
  rows.forEach((r) => {
    const ac = md.acIds(Object.values(r).join(' '))[0];
    if (!ac) return;
    ...
    seen.set(ac, {
      ...
      warn: /⚠️/.test(joined),
      // 근거 = 파일 경로에 줄번호가 붙은 것
      hasEvidence: /[\w/.-]+\.(ts|tsx|js|jsx|sql|sh):\d+/.test(joined) || /[\w/.-]+\.(ts|tsx|js|jsx|sql)/.test(joined),
    });
  });

  const missing = specAcs.filter((a) => !seen.has(a));
  if (missing.length) {
    out.push(problem(
      `${missing.join(', ')} 에 대한 대조가 없어요.`,
      '인수 조건 하나도 빠짐없이 판정해야 해요. 빠진 건 확인 안 한 것입니다.'
    ));
  }

  const failed = [...seen.entries()].filter(([, v]) => v.fail).map(([k]) => k);
  if (failed.length) {
    out.push(problem(
      `${failed.join(', ')} 가 아직 안 됐어요 (❌).`,
      '고친 뒤에 다시 대조해야 해요. 이대로 사용자에게 보내면 안 됩니다.'
    ));
  }

  const noProof = [...seen.entries()]
    .filter(([, v]) => !v.fail && !v.hasEvidence).map(([k]) => k);
  if (noProof.length) {
    out.push(problem(
      `${noProof.join(', ')} 에 근거가 없어요.`,
      '"잘 됩니다"는 근거가 아니에요. 어느 파일 몇 번째 줄인지 적어야 해요.\n' +
      '              근거를 못 대면 만들지 않은 것으로 봅니다.'
    ));
  }
...
const CHECKS = {
  security: checkSecurity,   // L0 — 되돌릴 수 없는 것
  spec: checkSpec,
  plan: checkPlan,
  evidence: checkEvidence,   // L3 — 근거를 못 대면 미구현
  verify: checkVerify,
};
...
function run(root, which) {
  ...
  // 통과·실패를 상태에 자동으로 남긴다.
  // `feature pass` 를 사람이 따로 불러야 하면 아무도 부르지 않는다.
  const FLAG = { security: 'securityPassed', evidence: 'evidenceChecked' };
  const touched = names.filter((n) => FLAG[n]);
  if (touched.length) {
    const failedGates = new Set(results.map((r) => r.gate));
    state.update(root, (s) => {
      const cf = s.currentFeature;
      if (!cf) return;
      touched.forEach((n) => { cf.gates[FLAG[n]] = !failedGates.has(n); });
    });
  }
...
module.exports = { run, checkSecurity, checkSpec, checkPlan, checkEvidence, checkVerify };
