...
function initialState(now) {
  ...
  return {
    ...
    // [{ id, title, status, dependsOn?: string[] }]
    // dependsOn 이 없으면 무엇이 영향받는지 알 수 없다 (CHANGE.md §4)
    backlog: [],
  };
}
...
function newFeature(id, title, now) {
  return {
    ...
    // 검증 준비물. 없으면 사용자가 확인 자체를 할 수 없다 (CHECKPOINTS §5).
    // 마크다운 표로 두면 파싱이 흔들린다 — 구조가 있는 것은 여기 둔다.
    prep: { accounts: [], seed: null, deeplink: null, note: null },

    // 인수 조건. [{ id, text }]
    // 사용자에게 그대로 보여주지 않는 **내부 검증 산출물**이라(CHECKPOINTS 차례8)
    // 마크다운에 둘 이유가 없다. 게이트 셋이 이걸 본다.
    acceptance: [],

    // L3 증거 대조. [{ ac, ok, proof }]
    // "잘 됩니다"를 막는 게 목적이라, proof 가 빈 항목은 통과하지 않는다.
    evidence: [],
  };
}
