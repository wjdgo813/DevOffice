...
| | |
|---|---|
| 버전 | **0.4.0** — 로드맵 v0.1 · v0.2 · v0.3 전부 완료 |
| 완료 | **Phase A~J ✅ (10개 전부)** |
| 진행 중 | — |
| 다음 작업 | **실사용에서 나오는 것**. 미리 만들지 않는다 |
| 마지막 갱신 | 2026-09-20 |

| 로드맵 | 무엇 | 플러그인 버전 |
|---|---|---|
| v0.1 | 배포된 첫 화면까지 | 0.1.0 |
| v0.2 | 기능 루프 (구체화·계약·구현·검증) | 0.3.0 |
| v0.3 | 회귀·수리 | 0.4.0 |

> **로드맵 번호와 플러그인 버전은 다르다.** 헷갈리기 쉬워 표로 남긴다.
...
|---|---|
| 상태 | **API 키 신청 중** |
| 지금 | `lib/jev.js` + `devoffice judge` 준비됨. **스킬에 연결 안 함** |
| 키 받으면 | `devoffice judge testset` → 17개 기대값과 비교 → 효과 판정 |
| 효과 있으면 | 스킬 연결 + 사용자 옵트인 경로 (문구 주의: [docs/jev.md](./docs/jev.md) §4단계) |
| 효과 없으면 | 붙이지 않는다 |

> **기대값을 먼저 적어뒀다.** 키를 받은 뒤 "좋아 보인다"로 판단하면 측정이 아니라 인상이다.

---

## 중간 산출물 — 무엇을 구조체로 두나

기준은 **"사용자가 보나"가 아니라 "기계가 파싱하나"** 다.
...
| `research/` | LLM + 사용자 | md | 긴 산문 |

> **왜 전부 구조체로 안 가나:** JSON 을 LLM 에게 쓰라고 하면 이스케이프 사고가 난다.
> 특히 한국어 긴 문장. 그리고 부분 수정이 위험하다 — 전체를 다시 쓰다 다른 필드를 날린다.
> **구조가 있는 것만 명령으로 기록**하고, 나머지는 마크다운으로 둔다.
>
> 손으로 만든 마크다운 파서(`lib/md.js`)는 이미 세 번 틀렸다.
> 그래서 **게이트가 파싱하는 항목을 최소로 줄였다.**
